...
class SDPController:
    ...
    def create_grids(self, R_bar: np.ndarray):
        ...
        # Pre-compute feasible actions for each SOC in grid (optimization)
        # This avoids repeated computation in the SDP backward loop
    ...
    def solve_sdp(self, R_bar: np.ndarray, verbose: bool = False):
        ...
        for k in range(N - 1, -1, -1):
            ...
            for i in range(n_x):
                ...
                # Ações viáveis (use pre-computed cache)
                U_i = self._feasible_actions_cache[i]
