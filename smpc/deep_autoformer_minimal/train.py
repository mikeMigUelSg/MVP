...
def main(cfg_path):
    ...
    # Enable parallel data loading for better GPU utilization
    num_workers = cfg["train"].get("num_workers", 4)  # Use 4 workers by default
    ...
    # GPU Optimizations
    use_compile = cfg["train"].get("compile_model", False)
    if use_compile and hasattr(torch, 'compile'):
        print("Compiling model with torch.compile for better GPU utilization...")
        model = torch.compile(model)
        print("  ✓ Model compiled")

    # Enable optimized memory format for better GPU performance
    if device.type == 'cuda':
